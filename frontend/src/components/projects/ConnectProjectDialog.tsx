...
export function ConnectProjectDialog({ nodeId, onSuccess }: ConnectProjectDialogProps) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api<ProjectActionResponse>('/api/projects/connect', {
        method: 'POST',
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={
        <Button variant="outline" size="sm">
          <Link size={14} className="mr-1" /> Connect
        </Button>
      } />
      <DialogContent>
        <DialogHeader>
