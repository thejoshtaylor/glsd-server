...
export function HistoryStreamPanel({ instanceId }: HistoryStreamPanelProps) {
  ...
  if (isLoading) return <div className="p-3 text-muted-foreground text-sm">Loading history...</div>
  if (error) return <div className="p-3 text-red-400 text-sm">Failed to load: {(error as Error).message}</div>
  if (!events?.length) return <div className="p-3 text-muted-foreground text-sm">No output recorded</div>

  return (
    <div className="flex flex-col h-full">
      <div className="p-3 border-b border-border bg-card/50">
        <div className="text-sm text-muted-foreground uppercase tracking-widest">
          History: <span className="text-muted-foreground font-mono text-xs">{instanceId.slice(0, 12)}...</span>
        </div>
      </div>
